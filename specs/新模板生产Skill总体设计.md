...
新 Skill 的正式生产交付物只有一份完成 URL 回填、通过正式 Schema 与业务质量检查的模板 JSON。阶段 sidecar、证据和清单只服务生产与审计，不交付给下游。主流程固定为 P0–P8。真实用户编辑与生图验证使用 `T1` 独立调用入口，单独读取已落盘 JSON；前端交互、后端实现和数据库导入在职责范围外。
