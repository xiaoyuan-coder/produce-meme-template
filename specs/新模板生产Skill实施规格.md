...
- 每次生产开始执行 doctor，核对安装版本、release digest、额外文件和合同兼容范围。检查失败时停止生产并给出稳定修复提示。
...
- 将历史经验矩阵中的 39 条规则逐项绑定至少一个绿 fixture 或红 fixture；迁移状态和验收证据缺失时禁止宣称能力迁移完成。
...
- 发布 `1.0.0` 前执行真实批次影子运行，覆盖普通真人、知名 IP、动物、文字梗、多实例、容器、穿戴、背景和混合媒介，并逐图核对模板图与最终 JSON。
...
- ADR 完整性审计已覆盖全部 46 条 Implementation Decisions；20 份 ADR 中 19 份生效，ADR 0007 已由 ADR 0008 取代。完整映射见《ADR 决策覆盖矩阵》；用户已确认当前决策前沿为空。
