...
`contracts/normative-rule-registry.json` 冻结每个权威文档的摘要，并为每个语义单元分别记录唯一 `ruleId`、摘要和执行家族。机器合同中的有序语义路由负责把混合文档中的换图、作者合同、T1、发布、OSS 等条款分配给对应家族；文档级 `familyId` 只提供未命中专门语义时的默认归属，不能覆盖语义单元的路由结果。每个执行家族必须同时绑定：
...
`--render` 只输出候选注册表，不写文件。维护者审查新增、删除和改写的 rule unit 及其执行家族后，再更新注册表。`--check` 会重新执行语义路由，精确对账当前权威集合、文档摘要、语义单元、逐单元家族、所有者和证据；把某个单元手工改回文档默认家族会直接失败。
