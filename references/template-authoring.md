...
主体槽采用强制优先级：当前图存在明显主体，且用户上传图能在保持机制的前提下接管该主体时，必须开放 `subject`。“当前 v2 不支持”、“为了简化控件”或自由文本理由不能省略主体。省略时必须提供 `uploadReplacementFeasible=false` 和机器合同允许的类型化 blocker；明显猫、狗、人物、单个道具等可分离主体通常不满足省略条件。`hasPrimarySubject` 不能单独决定门禁是否适用：P4 从 Approved 组件图的可见身份实例确定性重算主体存在性，并与 Authoring Handoff 的身份单元数量和绑定模式对账；独立作者审计还需直接复核 Approved Image、组件图、Handoff 与主体槽策略。任一方声明无主体或通过改写组件角色规避主体槽时均停止当前项。
