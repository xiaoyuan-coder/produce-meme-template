...
`fixtures/regression/historical-experience-matrix.json` 是 E01–E44 的机器追踪矩阵。每条经验只声明一个当前权威规则、一个实现定位、迁移状态、旧口径裁决和至少一条具体 unittest 证据。机器枚举、字段名、迁移状态、证据极性、corpus 角色、报告字段与失败分类只读取 `contracts/machine-rules.json` 的 `historicalExperienceContract`。
...
`scripts/release_validation_runner.py` 在完整单元、fixture、Schema 和 E2E 测试通过后编译历史经验报告；E01–E44、机器合同声明的全部 corpus 角色或任一版本绑定不完整时返回失败，release build 不会继续。安装副本运行同一报告入口时，production pin 来自该只读安装副本，因此源码 Git revision、发布清单、安装文件、pin 和报告可以逐摘要对账。
