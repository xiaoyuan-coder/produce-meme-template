...
`scripts/release_validation_runner.py` 在完整单元、fixture、Schema 和 E2E 测试通过后编译历史经验报告；40 条经验、九类 corpus 或任一版本绑定不完整时返回失败，release build 不会继续。安装副本运行同一报告入口时，production pin 来自该只读安装副本，因此源码 Git revision、发布清单、安装文件、pin 和报告可以逐摘要对账。
