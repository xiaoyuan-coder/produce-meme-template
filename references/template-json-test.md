...
- T1 将 `replace_content` 绑定编译为完整替换指令：先清除所有目标实例的旧内容与残留，再由当前输入按声明的分布策略接管；提示词末尾同时声明当前输入是这些内容目标的唯一事实源，清除目标区域内参考图的旧人物类型、旧服装、旧道具和旧活动语义，只保留目标实例、固定容器和视觉合同明确要求的媒介、画风特征、构图、关系与条件性色光。该优先级只作用于 `replace_content` 目标，不覆盖服装属于模板核心玩法时的身份替换裁决。
