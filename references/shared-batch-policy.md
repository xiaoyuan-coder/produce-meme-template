...
Artifact Schema `0.25.0` 起，每个 item 还必须拥有独立的 `template-key-resolution.json` 和 `authoring-contract-audit.json`。前者绑定该项 Source Image SHA 与注册表查询，后者绑定该项 Approved Image SHA、组件图、槽位候选和 Prompt 职责。批量内槽位数相同或命名相似不构成违规；任一项缺少自己的图片绑定与独立结论会单独阻断。
...
未提供 `sharedPolicy` 时，工作流将独立 Production Item 放入机器合同声明的五条并发通道。批次按用户大阶段设置屏障：全部可执行项完成第一阶段并冻结 Prompt 后，才允许任一项进入第二阶段；第二阶段对所有通过 P1 的项并发提交且每项只创建一个供应商请求。其后第三、四阶段分别在各自屏障后启动。跨项 adapter 调用可以重叠，归集结果继续保持输入顺序。批量不建立跨图候选池、不去重替换值、不共享版本或状态，也不使用其他 item 的视觉事实。单项失败只影响该 item；其他 item 继续生产。恢复、重做、上传和完成态对账也以 item 为边界。
